...
public class DirectoryBrowser {
    ...
    private static void createAndShowGUI() {
        ...
        // Use a small custom renderer to show system icons and file names
        tree.setCellRenderer((t, value, sel, expanded, leaf, row, hasFocus) -> {
            DefaultMutableTreeNode node = (DefaultMutableTreeNode) value;
            Component comp = new javax.swing.tree.DefaultTreeCellRenderer()
                    .getTreeCellRendererComponent(t, value, sel, expanded, leaf, row, hasFocus);

            Object userObj = node.getUserObject();
            if (userObj instanceof java.io.File) {
                java.io.File f = (java.io.File) userObj;
                String name = f.getName().isEmpty() ? f.getAbsolutePath() : f.getName();
                ((javax.swing.JLabel) comp).setText(name);
                ((javax.swing.JLabel) comp).setIcon(fsv.getSystemIcon(f));
            } else {
                ((javax.swing.JLabel) comp).setText(String.valueOf(userObj));
            }
            return comp;
        });
    ...
    private static void onWriteToFile() {
        ...
        saver.setDialogTitle("Save combined file as...");
        saver.setSelectedFile(new File("combined.txt"));
            ...
            protected Void doInBackground() {
                try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(out))) {
                    for (File f : selectedFiles) {
                        ...
                        String header = "----- Start of: " + f.getAbsolutePath() + " -----\n";
