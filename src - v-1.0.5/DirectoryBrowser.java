...
public class DirectoryBrowser {
    ...
    //private static final Color PRIMARY_COLOR = new Color(0, 137, 136);
    //private static final Color PRIMARY_COLOR = new Color(156, 39, 176);
    ...
    //private static final Color BUTTON_PRIMARY_HOVER = new Color(142, 35, 160);
    private static final Color BUTTON_PRIMARY_HOVER = new Color(36, 35, 37);
    //private static final Color BUTTON_PRIMARY_HOVER = new Color(57, 73, 171);
    //
...
// Set Material Design inspired theme
            UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());// Modern Material Design 3 color scheme (Blue theme)
            ...
            Color dividerColor = new Color(224, 224, 224);     // Grey 300// Customize UI defaults for Modern Material Design
            ...
            UIManager.put("Tree.foreground", textPrimary);// Font settings with Modern Material Design typography
            ...
            Font monoFont = new Font("Consolas", Font.PLAIN, 13);UIManager.put("Button.font", mediumFont);
            ...
            UIManager.put("OptionPane.font", regularFont);} catch (Exception e) {
            e.printStackTrace();
        }
        ...
        frame.setLayout(new BorderLayout(0, 0));// Top panel with modern Material Design styling
        JPanel topPanel = new JPanel(new BorderLayout(16, 16));
        topPanel.setBackground(new Color(18, 19, 20));
        ...
        topPanel.add(titleLabel, BorderLayout.WEST);// Path field and select button container
        JPanel pathPanel = new JPanel(new BorderLayout(8, 0));
        pathPanel.setBackground(new Color(18, 19, 20));
        ...
        selectButton.addActionListener(DirectoryBrowser::onSelectFolder);// Add hover effect to button
        selectButton.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                selectButton.setBackground(new Color(245, 245, 245));
                selectButton.setForeground(new Color(13, 71, 161));
            }
        ...
        topPanel.add(pathPanel, BorderLayout.CENTER);frame.add(topPanel, BorderLayout.NORTH);// Initial tree placeholder
        ...
        tree.setBackground(Color.WHITE);JScrollPane treeScroll = new JScrollPane(tree);
        ...
        //writeButton = new JButton("Write to Files");
        writeButton.setEnabled(false);
        writeButton.setFont(new Font("Segoe UI Semibold", Font.PLAIN, 14));
        /*writeButton.setBackground(new Color(25, 118, 210));
        writeButton.setForeground(Color.WHITE);
        writeButton.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createEmptyBorder(14, 28, 14, 28),
                BorderFactory.createLineBorder(new Color(25, 118, 210), 1)
        ));
        writeButton.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        writeButton.addActionListener(e -> onWriteToFile());// Add elevation effect to button
        writeButton.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                writeButton.setBackground(new Color(13, 71, 161));
                writeButton.setBorder(BorderFactory.createCompoundBorder(
                        BorderFactory.createEmptyBorder(14, 28, 14, 28),
                        BorderFactory.createLineBorder(new Color(13, 71, 161), 1)
                ));
            }
            public void mouseExited(java.awt.event.MouseEvent evt) {
                writeButton.setBackground(new Color(25, 118, 210));
        ...
        statusLabel.setBorder(BorderFactory.createEmptyBorder(0, 0, 8, 0));statusArea = new JTextArea();
        ...
        statusArea.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));statusPanel.add(statusLabel, BorderLayout.NORTH);
        statusPanel.add(new JScrollPane(statusArea), BorderLayout.CENTER);// Statistics panel
        ...
        statsPanel.add(filesLabel);statusPanel.add(statsPanel, BorderLayout.SOUTH);// Container for write button to center it
        JPanel buttonContainer = new JPanel(new FlowLayout(FlowLayout.CENTER));
        buttonContainer.setBackground(new Color(250, 250, 250));
        buttonContainer.add(writeButton);rightPanel.add(buttonContainer, BorderLayout.NORTH);
        rightPanel.add(statusPanel, BorderLayout.CENTER);// Split pane: tree (left) and rightPanel (right)
    ...
    private static Icon createFolderIcon() {
        return new Icon() {
            ...
            public int getIconWidth() { return 32; }@Override
...
// Create custom option pane with button on the right
            ...
            panel.setBorder(BorderFactory.createEmptyBorder(24, 24, 24, 24));// Create warning icon using graphics instead of loading from file
            JLabel iconLabel = new JLabel(createWarningIcon(48, 48));JPanel textPanel = new JPanel(new BorderLayout(8, 8));
            textPanel.setBackground(Color.WHITE);JLabel titleLabel = new JLabel("File Exists");
            titleLabel.setFont(new Font("Segoe UI Semibold", Font.BOLD, 16));
            titleLabel.setForeground(new Color(33, 33, 33));JLabel messageLabel = new JLabel("<html>The file '" + out.getName() + "' already exists. Do you want to replace it?</html>");
            messageLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
            messageLabel.setForeground(new Color(117, 117, 117));textPanel.add(titleLabel, BorderLayout.NORTH);
            textPanel.add(messageLabel, BorderLayout.CENTER);panel.add(iconLabel, BorderLayout.WEST);
            panel.add(textPanel, BorderLayout.CENTER);// Custom buttons on the right
            JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 8, 0));
            buttonPanel.setBackground(Color.WHITE);JButton noButton = new JButton("Cancel");
            ...
            ));final int[] result = {JOptionPane.NO_OPTION};noButton.addActionListener(ev -> {
            ...
            panel.add(buttonPanel, BorderLayout.SOUTH);JOptionPane optionPane = new JOptionPane(panel, JOptionPane.PLAIN_MESSAGE,
                    JOptionPane.DEFAULT_OPTION, null, new Object[]{}, null);JDialog dialog = optionPane.createDialog(frame, "File Exists");
            dialog.setVisible(true);
            dialog.dispose();if (result[0] != JOptionPane.YES_OPTION) {
                return;
            }
        }appendStatus("📁 Saving combined file to: " + out.getAbsolutePath());
        writeButton.setEnabled(false);
        writeButton.setText("Combining Files...");// Background writing using SwingWorker
...
// Header format: /** FileName /
            ...
            protected void done() {
                writeButton.setEnabled(true);
                writeButton.setText("Combine Selected Files");
    ...
    private static Icon createWarningIcon(int width, int height) {
        return new Icon() {
            ...
            public void paintIcon(Component c, Graphics g, int x, int y) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);// Draw warning triangle
                ...
                g2.drawPolygon(xPoints, yPoints, 3);// Draw exclamation mark
                ...
                g2.drawString(exclamation, x + (width - textWidth)/2, y + (height + textHeight)/2 - 8);g2.dispose();
            }@Override
    ...
    private static JFileChooser createMaterialFileChooser(String title) {
        JFileChooser chooser = new JFileChooser() {
            ...
            protected JDialog createDialog(Component parent) throws HeadlessException {
                ...
                dialog.setBackground(Color.WHITE);// Apply Modern Material Design styling to all components
                SwingUtilities.invokeLater(() -> {
        ...
        chooser.setForeground(new Color(33, 33, 33));return chooser;
    ...
    private static void applyMaterialStyling(Container container) {
        for (Component comp : container.getComponents()) {
            if (comp instanceof JButton) {
                ...
                button.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));// Add hover effect
    ...
    private static void showMaterialDialog(String title, String message, int messageType) {
        JPanel panel = new JPanel(new BorderLayout(16, 16));
        panel.setBackground(Color.WHITE);
        panel.setBorder(BorderFactory.createEmptyBorder(24, 24, 24, 24));// Create icon based on message type
        Icon icon = createMaterialIcon(48, messageType);
        JLabel iconLabel = new JLabel(icon);JPanel textPanel = new JPanel(new BorderLayout(8, 8));
        textPanel.setBackground(Color.WHITE);JLabel titleLabel = new JLabel(title);
        titleLabel.setFont(new Font("Segoe UI Semibold", Font.BOLD, 16));
        titleLabel.setForeground(new Color(33, 33, 33));JTextArea messageArea = new JTextArea(message);
        ...
        messageArea.setBorder(BorderFactory.createEmptyBorder());textPanel.add(titleLabel, BorderLayout.NORTH);
        textPanel.add(messageArea, BorderLayout.CENTER);panel.add(iconLabel, BorderLayout.WEST);
        panel.add(textPanel, BorderLayout.CENTER);JOptionPane optionPane = new JOptionPane(panel, messageType, JOptionPane.DEFAULT_OPTION);
    ...
    private static Icon createMaterialIcon(int size, int messageType) {
        return new Icon() {
            ...
            public void paintIcon(Component c, Graphics g, int x, int y) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);Color iconColor;
                String symbol;if (messageType == JOptionPane.ERROR_MESSAGE) {
                ...
                g2.fillOval(x, y, size, size);// Draw symbol
                g2.setColor(Color.WHITE);
                g2.setFont(new Font("Segoe UI", Font.BOLD, size/2));
                FontMetrics fm = g2.getFontMetrics();
                int textWidth = fm.stringWidth(symbol);
                int textHeight = fm.getAscent();
                g2.drawString(symbol, x + (size - textWidth)/2, y + (size + textHeight)/2 - 4);g2.dispose();
            }@Override
            public int getIconWidth() {
                return size;
            }@Override
